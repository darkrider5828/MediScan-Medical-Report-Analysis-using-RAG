...
ANALYSIS_MODEL_NAME = "gemini-2.0-flash" # Model for main analysis & table extraction
CHAT_MODEL_NAME = "gemini-1.5-flash"     # Model specifically for the chat feature
...
app.config['SESSION_FILE_DIR'] = './flask_session'  # Folder for session files
app.config['SESSION_PERMANENT'] = True  # Sessions persist until cleared
app.config['PERMANENT_SESSION_LIFETIME'] = 86400  # 1 day expiration (optional)
...
def chat():
    ...
    try:
        ...
        try:
            ...
            raise IOError(f"Internal error reading content: {type(e).__name__}")

        # --- Process Query (unchanged) ---
        if 'chat_session_data' not in session: 
            session['chat_session_data'] = initialize_chat()
            session['chat_history'] = []
        ...
        # --- Update History (now without size limits!) ---
    ...
    # --- Error handling (unchanged) ---
        ...
        print(f"!!! Unhandled Error in /chat route: {error_type}: {e} !!!")
        traceback.print_exc()
...
def visualize():
    ...
    csv_path = session.get('csv_file_path')
    if not csv_path:
        print("Exiting /visualize: No CSV path in session.")
        return jsonify({"error": "No extracted table data found."}), 404
        
    if not os.path.exists(csv_path):
        print(f"Exiting /visualize: CSV not found at {csv_path}")
        session.pop('csv_file_path', None)
        session.modified = True
        return jsonify({"error": "Table data file missing. Re-analyze."}), 404

    try:
        print(f"Generating visualizations from: {csv_path}")
        plots_dict = generate_visualizations(csv_path)
        
        if plots_dict is None:
            print("Exiting /visualize: generate_visualizations returned None.")
            return jsonify({"error": "CSV file error during visualization."}), 404
        elif not plots_dict:
            print("Exiting /visualize: No suitable data found.")
            return jsonify({"plots": {}, "message": "No data suitable for visualization."})
        else:
            print(f"Exiting /visualize: Returning {len(plots_dict)} plot(s).")
            return jsonify({"plots": plots_dict})
            
    except pd.errors.EmptyDataError:
        print(f"Exiting /visualize: EmptyDataError: {csv_path}")
        return jsonify({"error": "Extracted table data empty/invalid."}), 404
    except Exception as e:
        print(f"Error in /visualize: {e}")
        traceback.print_exc()
        print("Exiting /visualize: Unexpected error.")
        return jsonify({"error": f"Unexpected visualization error: {type(e).__name__}"}), 500
