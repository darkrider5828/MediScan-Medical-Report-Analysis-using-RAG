    ...
    if (visualizeBtn && visualizationsDiv && visualizationContent && vizSpinner) {
        visualizeBtn.addEventListener('click', async () => {
            ...
            if (isHidden) {
                ...
                showSpinner(vizSpinner, 'Loading Visualizations...');
                setControlsDisabled(true);
                ...
                try {
                    ...
                    if (!response.ok) {
                        throw new Error(data?.error || `Failed to load visualizations (Status: ${response.status})`);
                    }

                    if (data.plots) {
                        visualizationContent.innerHTML = ''; // Clear previous content
                        
                        // Create container divs for each plot
                        if (data.plots.pie_chart) {
                            const pieContainer = document.createElement('div');
                            pieContainer.id = 'pie-chart-container';
                            pieContainer.style.width = '100%';
                            pieContainer.style.height = '400px';
                            visualizationContent.appendChild(pieContainer);
                            Plotly.newPlot('pie-chart-container', data.plots.pie_chart.data, data.plots.pie_chart.layout);
                        }
                        
                        if (data.plots.bar_chart) {
                            const barContainer = document.createElement('div');
                            barContainer.id = 'bar-chart-container';
                            barContainer.style.width = '100%';
                            barContainer.style.height = '400px';
                            visualizationContent.appendChild(barContainer);
                            Plotly.newPlot('bar-chart-container', data.plots.bar_chart.data, data.plots.bar_chart.layout);
                        }
                        
                        setVisualizeButtonState(true);
                    } else {
                        visualizationContent.innerHTML = '<p class="info-text">No visualizations available.</p>';
                    }
                } catch (error) {
                ...
                visualizationContent.innerHTML = '';
                setVisualizeButtonState(false);
            }
        });
    }

    function setVisualizeButtonState(isVisible) {
        if (!visualizeBtn) return;
        visualizeBtn.innerHTML = isVisible 
            ? '<span class="icon">📉</span> Hide Visuals' 
